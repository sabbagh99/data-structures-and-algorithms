...
public class AppTest {
  ...
  public void testApp() {
    ...
    assertEquals("[5, 3, 7]", tree.preOrder(binarySearchTree.root).toString());//Can successfully add a left child and right child to a single root node
    tree.arr.clear();
    binarySearchTree.add(1);
    binarySearchTree.add(9);

    assertEquals("[1, 3, 5, 7, 9]", tree.inOrder(binarySearchTree.root).toString());//Can successfully return a collection from an inorder traversal

    tree.arr.clear();
    assertEquals("[5, 3, 1, 7, 9]", tree.preOrder(binarySearchTree.root).toString());//Can successfully return a collection from a preorder traversal

    tree.arr.clear();
    assertEquals("[1, 3, 9, 7, 5]", tree.postOrder(binarySearchTree.root).toString());//Can successfully return a collection from a postorder traversal
  ...
  public void testBinaryTree(){ // testing the max value for the binary  tree
    ...
    assertEquals(0,binaryTree.traverseTree(binaryTree.root));
    binaryTree.root = new Node(1);
    assertEquals(1,binaryTree.traverseTree(binaryTree.root));
    binaryTree.root.left = new Node(7);
    binaryTree.root.right = new Node(-5);
    assertEquals(7,binaryTree.traverseTree(binaryTree.root));
    binaryTree.root.left.left = new Node(10);
    assertTrue(binaryTree.traverseTree(binaryTree.root)==10);
  }
